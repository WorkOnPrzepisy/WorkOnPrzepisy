...
const store = new MongoDBStore({
   ...
   uri : 'mongodb+srv://WorkOnPrzepisy:2021WorkOnFrontend@cluster0.pmwld.mongodb.net/eatme?retryWrites=true&w=majority',
...
app.post('/users/favorites', async (req, res) => {
   ...
   const apiName = (array, strName) => {
      ...
      for (var i = 0; i < array.length; i++) {
         if (array[i].strMeal.toLowerCase().includes(strName)) {
            return array[i]
         }
      }
   }
   const obj = {
      key: await findKey(dataBaseResultsName, firstLetterCapitalize),
